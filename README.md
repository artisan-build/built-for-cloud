...
Built for Cloud commands, queued jobs/listeners, package-registered schedule callbacks, and unbound
installation MCP requests execute in a package system-authority context. An unbound installation MCP request is framed during immediate downstream dispatch and again while a returned streamed response callback executes. Commands and queued entries are framed at their own invocation — the
