...
redirected by a display name. A queued entry's own `middleware()` and its `failed()` handler are inside the frame. The queue frame is released on `JobAttempted`, which the worker and the sync driver each dispatch from a `finally`. It is deliberately NOT released on `JobProcessed`, `JobFailed` or `JobExceptionOccurred`: each of those fires while package code can still run.
