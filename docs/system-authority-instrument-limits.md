...
redirected by a display name. A queued entry's `failed()` handler is inside the frame, as is the middleware a queued entry runs: for a job that is its own `middleware()` method, and for a listener it is the middleware objects that method returns. Anything the framework calls on an entry while DISPATCHING OR PUSHING it runs in the dispatcher's context, not the entry's, for a job and for a listener alike — a job's `displayName`, `backoff`, `retryUntil` and `tries`, and a listener's `middleware()` method body, `shouldQueue`, `viaConnection`, `viaQueue`, `withDelay`, `messageGroup`, `uniqueId` and the rest are examples rather than the boundary. They are framed only when the code dispatching or pushing the entry is itself framed. The queue frame is released on `JobAttempted`, which the worker and the sync driver each dispatch from a `finally`. It is deliberately NOT released on `JobProcessed`, `JobFailed` or `JobExceptionOccurred`: each of those fires while package code can still run.
