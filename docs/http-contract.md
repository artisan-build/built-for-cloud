...
**api_version 2** (bfc **0.12.0**, this release). All changes since version 1, in one inventory.
Additive unless marked otherwise.

**Everything the Console adds through 0.12.0 is additive, so `api_version` stays 2. What carries the
signal is `bfc_version` 0.12.0 plus the `capabilities` entries** — `console-keys`,
