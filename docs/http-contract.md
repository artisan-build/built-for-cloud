...
**api_version 2** (bfc **0.6.3**, this release). All changes since version 1, in one inventory.
Additive unless marked otherwise.

**Everything the Console adds in 0.6.3 is additive, so `api_version` stays 2. What carries the
signal is `bfc_version` 0.6.3 plus the `capabilities` entries** — `console-keys`,
