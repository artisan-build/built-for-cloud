 ...
 * ONE place.
 *
 * They are the default vocabulary, not the only permissible one: an
 * endpoint whose field is bounded in a charset of its own pins that
 * charset in its OWN schema instead — the console `kid`
 * (`[A-Za-z0-9._-]{1,64}`) is the one such field today. Loosening these
 * patterns to accommodate it would have let a capitalised word through
 * everywhere, which is the whole failure they exist to prevent.
 *
