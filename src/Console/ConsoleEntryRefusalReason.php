...
/**
 * Why a delegated-assertion DOOR refused a presentation for a reason
 * the ASSERTION ITSELF was not refused for (Console PRD D12/D13) — a
 * bounded, machine-readable vocabulary for the AUDIT RECORD, never for
 * the caller.
 *
 * ONE door decides in this vocabulary: the MCP authentication
 * middleware `AuthenticateMcp`, which renders its uniform 401. The
 * enum's Console-prefixed NAME predates the retirement of the
 * delegated-entry door and is kept for stability; the state and
 * return-path cases survive from that door, which was the only one that
 * could produce them, and remain so that historical audit records still
 * resolve against the vocabulary they were written with.
 *
 ...
 * verified — purpose, the single-use burn and containment. One audit
 * note therefore carries exactly one value from exactly one of the two
 * vocabularies, and a reader never has to disambiguate.
 *
 ...
 * {@see AssertionRefusalReason} — into ONE response, so a presenter at
 * the door cannot tell a replay from a wrong audience from a bad
 * signature.
 */
