 ...
 * 7. a replayed nonce — checked FIRST among the post-signature rules and
 *    spending NO rate budget: a replay is not a new accepted
 *    verification, so replaying one captured envelope N times can never
 *    rate-limit the legitimate holder. The nonce is consulted ONLY
 *    AFTER the signature verified — earlier would let an attacker burn
 *    a victim's nonce with a garbage signature;
 * 8. a per-key rate ceiling on GENUINELY NEW accepted verifications
 ...
 *    of the key can spend its budget — and before the new nonce is
 *    stored, so one credential can never grow the nonce store past its
 *    ceiling.
 *
...
final class HmacVerifier
{
    ...
    public function verify(Subject $subject, string $header, string $body, ?string $audience = null): Credential
    {
        ...
        // Replay FIRST, and a replay spends NO budget (rework Fix 2): a
        // replayed envelope is not a new accepted verification, so an
        // attacker replaying one captured message cannot exhaust the
        // legitimate holder's rate ceiling.
        if (Cache::has($nonceKey)) {
            throw HmacVerificationFailed::replayedNonce();
        }

        // The cardinality bound: GENUINELY NEW accepted verifications per
        // key per window. Only signature-valid requests reach this
        // counter, so nobody without the key can spend a credential's
        // budget — and because it is checked BEFORE the new nonce is
        // stored, the nonce store holds at most `ceiling` entries per key
        // per window.
        ...
        if (! Cache::add($nonceKey, 1, $windowSeconds)) {
            // A concurrent duplicate landed between the has() and this
            // add(): it is a replay after all — refund the budget unit so
            // replays never spend one.
