 ...
 *
 * WHAT THESE SCANS DO NOT DO, said here rather than left to be
 * discovered, because a scanner's blind spots are a claim too:
 *
 *  - {@see unclassifiedRoutes()} checks that a documented route HAS a
 *    classification, never that the classification is CORRECT. A route
 *    returning free text under a `metadata` row passes. The
 *    truth of a row against a real response is
 ...
 *  - It sees only routes the document declares with a `### METHOD /path`
 *    heading. A route that exists and is documented nowhere is caught by
 *    the route-completeness checks instead, against the router.
 *  - {@see capabilitiesNotNamedIn()} checks that a capability's NAME
 *    occurs in the document. It does not check that what the document
 *    says about it is true, nor that the condition under which the
 *    package emits it matches the predicate the document states — those
 *    are driven behaviourally, per capability, in the Console suites.
...
final class ContractScan
{
    ...
    /**
     * The classification table, as route => `metadata` | `content`.
     *
