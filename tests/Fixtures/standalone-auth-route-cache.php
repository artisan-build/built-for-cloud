...
$case = new class('testProbe') extends TestCase
{
    ...
    public function runProbe(string $payload, string $vector): bool
    {
        ...
        return $allRefused
            && ($vector === 'fqcn-alias' || ($poisonedStacks === 17 && ! in_array(false, $recomputed, true)))
...
if ($vector === 'fqcn-alias') {
    fwrite(STDOUT, "standalone-auth-route-cache-refused-17\n");
} else {
    fwrite(STDOUT, "standalone-auth-route-cache-{$vector}-refused-17\n");
}
