...
$case = new class('testProbe') extends TestCase
{
    ...
    public function runProbe(): bool
    {
        ...
        return $allRefused
            && (! $memoPoisoning || ($poisonedStacks === 16 && ! in_array(false, $recomputed, true)))
...
if (in_array($vector, ['memo-set-action', 'memo-property'], true)) {
    fwrite(STDOUT, "standalone-auth-gate-{$vector}-refused-16\n");
} else {
    fwrite(STDOUT, "standalone-auth-gate-match-refused-16\n");
}
